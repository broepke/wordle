...
letter_counts_df = pd.DataFrame.from_dict(
    letter_counts, orient="index", columns=["Count"]
).reset_index()
...
chart = (
    alt.Chart(letter_counts_df)
    .mark_bar()
    .encode(x=alt.X("Letter", sort=None), y="Count", color=alt.value("#F63366"))
